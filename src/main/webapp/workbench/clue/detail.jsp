...
<script type="text/javascript">
	...
	$(function(){
        ...
        $("#selectActivityByName").keydown(function (data) {
            if(data.keyCode==13){
                var $name= $("#selectActivityByName").val();
                // alert("发送请求")
                $.ajax({
                    url:"workbench/clue/selectActivityByName.do",
                    type:"post",
                    dataType:"json",
                    data:{
                        "name":$name,
        ...
        $("#guanlianBtn02").click(function () {
            var $allXZ= $("input[name=xz]:checked")
            if($allXZ.length==0){
                alert("请选择关联项")
            }else{
                var parment="";
               for (var i=0;i<$allXZ.length;i++){
                   parment+="aid="+$($allXZ[i]).val();
                   if (i<$allXZ.length-1) {
                       parment+="&"
                   }
               }
               parment="workbench/clue/addGuanLian.do?cid="+"${c.id}&"+parment;
               $.ajax({
                   url:parment,
                   type:"post",
                   dataType:"json",
	...
	function tableSX(){
		$.ajax({
			url:"workbench/clue/detailGetListActivityById.do",
			type:"post",
			dataType:"json",
			data:{
				"id":"${c.id}"
			...
			success:function (data) {
				...
				$.each(data,function (i,n) {
					...
					html+='<td>'+n.endDate+'</td>'
					html+='<td>'+n.owner+'</td>'
					html+='<td><a href="javascript:void(0);"  style="text-decoration: none;" onclick="guanlianguanxi(\''+n.id+'\')"><span class="glyphicon glyphicon-remove"></span>解除关联</a></td>'
					html+='</tr>'
				})
...
<body>
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
					...
					<div class="btn-group" style="position: relative; top: 18%; left: 8px;">
						<form class="form-inline" role="form">
						  <div class="form-group has-feedback">
						    <input type="text" id="selectActivityByName" class="form-control" style="width: 300px;" placeholder="请输入市场活动名称，支持模糊查询">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-primary" data-dismiss="modal" id="guanlianBtn02">关联</button>
				</div>
			</div>
		</div>
	</div>
	...
	<div>
		<div style="position: relative; top: 60px; left: 40px;">
			...
			<div>
				<a href="javascript:void(0);" id="guanlianBtn" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
